...
def find_intents(question, intent_list, user_info=None, faq_text=None, chat_session=None):
    ...
    prompt = f"""
        ...
        Bạn sẽ nhận được một đoạn hội thoại và câu hỏi mới từ người dùng. Nhiệm vụ của bạn là xác định intent phù hợp nhất với câu hỏi mới dựa trên ngữ cảnh đã có.
...
def generate_answer(question, context, user_info=None, chat_session=None):
    ...
    - Bạn luôn trả lời ngắn gọn, chính xác và dễ hiểu.
    - Các từ về chuyên ngành, chức vụ vị trí, tên người phải được giữ nguyên.   
    - Nếu người dùng hỏi về học phí, hãy hướng dẫn họ đến chatbot của tư vấn viên và vị trí là ngay dưới chatbot này.
