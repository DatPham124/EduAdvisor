...
const ChatbotContent = ({ registerAd }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    const tempMessage = { question: userQuestion, answer: '...' };
    ...
    recognition.onresult = (event) => {
      ...
      if (input.length > 20) {
        ...
        if (langMap[langCode] && langMap[langCode] !== detectedLang) {
          setDetectedLang(langMap[langCode]);
        }
      }
    };
  ...
  return (
    <div className="flex flex-col h-[90vh] max-h-[60vh]">
      <div
        ref={messagesEndRef}
        className="flex flex-col space-y-2 overflow-y-auto h-full p-2 bg-white rounded"
      >
        <div className="flex justify-start text-sm text-gray-700 ">
          <div className="bg-gray-100 p-3 rounded-lg shadow">
            Xin chào {registerAd?.name || 'bạn'}, tôi có thể giúp gì cho bạn?
          </div>
        </div>
        ...
        {messages.map((msg, idx) => (
          <div key={idx}>
            <div className="flex justify-end text-sm text-white">
              <div className="bg-primary p-3 rounded-lg shadow max-w-[80%] my-2">
                {msg.question}
              </div>
            </div>
            <div className="flex justify-start text-sm text-black">
              <div className="bg-gray-100 p-3 rounded-lg shadow max-w-[80%] mt-2">
                {msg.answer === '...'
                  ? <span className="animate-pulse">Đang phản hồi...</span>
                  : msg.answer}
              </div>
              <button
      ...
      <form onSubmit={handleSendMessage} className="mt-2 flex items-center">
        <input
          ...
          className="flex-grow chat-input rounded-l-md text-black"
          required
        />
        <button
          type="submit"
          className="bg-primary text-white chat-send-btn rounded-r-md"
          disabled={isSending}
        >
          {isSending ? '...' : 'Gửi'}
        </button>
